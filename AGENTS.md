...
- **Stay timeless.** This file and the `docs/` specification hold only durable content — implementation status lives in the catalog's `live`/`target` rows, the V&V tiers (`vv/`), CI, and git; never in a narrative status doc.
