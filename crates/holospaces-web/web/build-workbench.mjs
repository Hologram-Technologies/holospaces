...
export function runtimeConfig(extensionIds) {
  ...
  return `<script>(function(){var loc=window.location;var dir=loc.pathname.replace(/\\/[^/]*$/,"");var declared=${ids};var sp=new URLSearchParams(loc.search);var q=sp.get("ext");if(q){declared=declared.concat(q.split(",").filter(Boolean));}var id=sp.get("id")||"";var eg=sp.get("egress")||"";var gallery=declared.map(function(id){return {id:id};});var cfg={folderUri:{"$mid":1,scheme:"holospace",authority:id,path:"/workspace",query:eg?("egress="+encodeURIComponent(eg)):""},additionalBuiltinExtensions:[{scheme:loc.protocol.replace(":",""),authority:loc.host,path:dir+"/ext/holospace-fs"}].concat(gallery),productConfiguration:{nameShort:"holospaces VS Code",nameLong:"holospaces VS Code",applicationName:"code-web",version:"1.91.1",extensionsGallery:{serviceUrl:"https://open-vsx.org/vscode/gallery",itemUrl:"https://open-vsx.org/vscode/item",resourceUrlTemplate:"https://open-vsx.org/vscode/unpkg/{publisher}/{name}/{version}/{path}"}}};document.getElementById("vscode-workbench-web-configuration").setAttribute("data-settings",JSON.stringify(cfg));})();</script>`;
}
