...
try {
  ...
  if (!r.ok) {
    ...
    check(r.imageLen >= r.disk && r.onDiskLen === r.imageLen,
      `the real image provisioned SPARSE into OPFS via the deployed assembleIntoOpfs (${r.imageLen} bytes for a ${r.disk}-byte disk)`);
    check(r.mounted, "a real Linux mounted the provisioned rootfs over virtio-blk — in the browser");
    check(r.booted, "the registry-pulled image BOOTED to userspace via the deployed provision→paged-boot path (holospace devcontainer ready)");
    if (!r.booted) console.error("  console tail:\n" + r.tail);
  }
  console.log(failed
    ? "PROVISION-BOOT-TEST: FAILED"
    : "PROVISION-BOOT-TEST: PASS (a registry image provisioned via the deployed sparse path booted to userspace)");
} finally {
  await browser.close();
  server.close();
}
