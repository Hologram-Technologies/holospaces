...
tier. holospaces removes the VM: in the Platform Manager you import the
devcontainer and **launch** it — a **workspace projection** (editor,
file tree, terminal) opens in a new tab over the running devcontainer.
Its operating system is computed by a **κ-addressed system emulator** on
the hologram substrate (ADR-009): the OS image is content, I/O is
canonical events on channels, state is a κ snapshot — same lifecycle
(boot, suspend, resume, migrate), no Docker daemon, no cloud host, no
server. The browser is the compute substrate; the only limits are the
device’s. This is the execution surface of Chapter 9 (ADR-009) and
Chapter 11 (RT1), witnessed against the [Linux](https://www.kernel.org)
and [Dev Container](https://containers.dev) specifications (Chapter 10).
...
| R7  | Run an **arbitrary operating system** in a holospace — a real [Linux](https://www.kernel.org) dev environment and its real binaries (the devcontainer’s image), computed over the substrate.                                                      |
| R8  | Open a **workspace projection** — a browser editor, file tree, and terminal over a running holospace — the [Codespaces](https://github.com/features/codespaces) / [Gitpod](https://www.gitpod.io) experience, launched from the Platform Manager. |
