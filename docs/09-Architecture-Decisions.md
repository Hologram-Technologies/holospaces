...
**Status:** Accepted. (Realizes the *Projecting* conceptual model —
Managing and Working, OPM SD4 — and Chapter 11 RT3.)

**Context:** The operator manages holospaces the way they manage
resources on any infrastructure platform — sign in, organize, and
perform lifecycle operations — and *enters* a holospace to work in it.
The conceptual model (SD4) names these **Managing** and **Working**; the
building blocks (Chapter 5) name them the **Platform Manager** and the
**Workspace Projection**. The question is what realizes each, faithfully
and verifiably.

- **Managing.** A bespoke list of κ-labels is not a management console.
  The operator needs a console with the same shape as existing
  platform/infrastructure managers: an authenticated session, their
  holospaces organized and shown with status, and the lifecycle
  operations (provision · enter · suspend · resume · terminate) as
  first-class actions. The console is a **projection** (Law L3): it
  holds no state of its own — the canonical state is the operator’s
  holospaces and roster (κ in the store), and each action composes a
  path already witnessed against an external authority (identity `CC-1`;
  provisioning `CC-4`; boot `CC-9`; the workspace `CC-11`).

- **Working.** "The Codespaces/Gitpod experience" is concrete: it is the
  **VS Code** editor and terminal. Hand-rolling an editor and a terminal
  would be neither the real experience nor verifiable against a
  standard. The authentic, verifiable realization uses the **actual
  components VS Code, Codespaces, and Gitpod are built from** — the
  [Monaco](https://microsoft.github.io/monaco-editor/) code editor and
  the [xterm.js](https://xtermjs.org) terminal — as external, pinned,
  integrity-verified artifacts, bound to a **running holospace**:
  xterm.js is the terminal over the holospace’s console channel (the
  operator’s input published as canonical events, `CC-11`); Monaco is
  the editor over the environment’s content read and written **by κ**
  (`CC-11`); the file tree lists that content. The environment is a
  validated devcontainer (`CC-4`), booted on the system emulator
  (`CC-9`).

**Decision:** The **Platform Manager** is a management console — sign in
(a self-sovereign identity, `CC-1`), organize the operator’s holospaces,
and drive their lifecycle — delivered from the cold-start gateway
(GitHub Pages) as the browser peer. **Entering** a holospace launches
the **Workspace Projection** in a new browser tab: a VS Code-shaped IDE
built from the real Monaco editor and xterm.js terminal, bound to the
running holospace over the substrate — the editor and file tree read and
edit the environment’s content by κ, and the terminal drives the booted
OS (its commands canonical events advancing the κ snapshot). The console
and the workspace are projections (Law L3); the editor and terminal
libraries are external authorities, pinned and verified by re-derivation
on load like any imported artifact (Law L5; provenance in
`vv/PROVENANCE.md`). Realized by the Platform Manager and Workspace
Projection building blocks (Chapter 5) and witnessed by `CC-12` (the
management console) and `CC-13` (the VS Code workspace) in Chapter 10.

**Consequences:** The operator gets a real management platform and the
real VS Code experience, not an approximation — and every part traces to
an external standard a witness checks: the identity hashes (`CC-1`), the
Dev Container spec (`CC-4`), the Linux/QEMU OS behaviour (`CC-9`), the
projection surfaces (`CC-11`), and the VS Code editor + terminal
components themselves (Monaco + xterm.js, pinned). No server renders the
IDE; the browser peer is the machine, and the workspace it drives is
content through the substrate (Laws L1, L3, L4).

## ADR-011: The devcontainer is a real operating system on VirtIO devices over the substrate

**Status:** Accepted. (Extends ADR-009; realizes the *Working*
conceptual model with full devcontainer fidelity.)

**Context:** A complete
[Codespaces](https://github.com/features/codespaces) /
[Gitpod](https://www.gitpod.io) environment is a **real operating
system** with a real toolchain — `bash`, the coreutils, `git`, language
runtimes, and a package manager (`apt`) — running the repository’s
`devcontainer.json` (`features`, `postCreateCommand`). The CC-9 boot
proves an unmodified kernel runs on the emulator, but it boots a minimal
initramfs with a single static shell; there is no real root filesystem,
no real binaries, and no way for the editor to share the running
environment’s files. Reaching full fidelity requires the OS to have
**devices**: a block device for the root filesystem, a shared-folder
device the editor binds, and a network device for the package manager.

- The device interface must be a **published standard**, not a bespoke
  one, so it is conformance-checkable against an external authority and
  the same kernel that runs on real hardware and QEMU runs here
  unmodified. The standard is the OASIS [VirtIO
  specification](https://docs.oasis-open.org/virtio/virtio/v1.2/virtio-v1.2.html)
  (v1.2) — `virtio-mmio` transport with `virtio-blk` (block),
  `virtio-9p` (shared folder, the
  [9P2000.L](https://ericvh.github.io/9p-rfc/rfc9p2000.l.html)
  protocol), and `virtio-net` (network) devices.

- Every device’s backing store is **κ-addressed content** over the
  substrate (Law L1, L4): `virtio-blk` is backed by the `CC-7` κ-disk;
  the root filesystem is a real distribution image ingested as κ content
  (`CC-10` OCI ingestion → `CC-7` ext4 κ-disk); `virtio-9p` serves the
  workspace’s files as κ content (the editor and the OS share them);
  `virtio-net` frames are content on a channel. No device introduces a
  located resource.

**Decision:** The emulator implements the **VirtIO** device transport
and devices (`virtio-mmio` + `virtio-blk` / `virtio-9p` / `virtio-net`),
each conformance-checked against the VirtIO specification and bound to
substrate content. The devcontainer boots a **real distribution root
filesystem** — a published OCI base image (e.g. Debian) ingested by κ
(`CC-10`) into an ext4 `CC-7` κ-disk, mounted over `virtio-blk` as `/` —
so the terminal is a real OS with real binaries. The workspace’s files
are a **shared filesystem** the OS mounts over `virtio-9p` (9P2000.L)
and the editor reads and writes by κ — editing a file in the editor and
running it in the terminal are the same content. The package manager and
`devcontainer.json` `features` reach the network over `virtio-net`. The
boot still hands off as the SBI firmware (`CC-9`); the devices are
declared in the device tree. This is realized in phases, each witnessed
against its published spec (Chapter 10): `CC-14` VirtIO block + the real
rootfs, `CC-15` the VirtIO-9p shared workspace filesystem, `CC-16`
VirtIO networking + the package manager.

**Consequences:** The devcontainer is a real OS the operator’s
repository targets, not a toy shell, and every device speaks a published
standard a witness validates — the same kernel image runs unmodified on
the emulator, on real hardware, and under QEMU (the differential
oracle). The root filesystem, the shared files, and the network are all
content through the substrate (Laws L1, L4); identity stays content,
never location. The cost is the device models and a real-rootfs build
pipeline, each landed and witnessed in turn.

## ADR-012: The workspace is the real VS Code web workbench, bridged to the holospace over the substrate

**Status:** Accepted. (Supersedes the editor/terminal realization of
ADR-010 for the workspace; the management console of ADR-010 stands.)

**Context:** "Extensions and the other features expected of a
Codespace/Gitpod" are **VS Code’s**: the workbench (activity bar,
command palette, settings, source control, panels), the **extension
host** and its extensions, and the language and debug intelligence.
ADR-010 rendered the workspace from the Monaco editor and the xterm.js
terminal — VS Code’s **components**, but not its workbench, so there are
no extensions, no command palette, no language servers. The complete
experience is the **real VS Code web workbench** (the build that runs
`vscode.dev`), embedded in the browser peer and bridged to the running
holospace.

- The workbench’s backends are defined by **published protocols and
  APIs**: the VS Code
  [`FileSystemProvider`](https://code.visualstudio.com/api/extension-guides/virtual-documents)
  API (the editor’s files), the workbench **terminal** API (`pty`), the
  [Language Server
  Protocol](https://microsoft.github.io/language-server-protocol/) (code
  intelligence), and the [Debug Adapter
  Protocol](https://microsoft.github.io/debug-adapter-protocol/)
  (debugging). holospaces **implements these providers over the
  substrate**: the filesystem provider is the `virtio-9p` workspace
  (`CC-15`, content by κ); the terminal is the holospace’s console
  (`CC-11`); the language servers and debug adapters run **in the
  devcontainer OS** (`CC-14`), reached over the providers.

- The workbench and its extensions are **imported by κ and verified by
  re-derivation** before they load (Law L5; ADR-010’s `importKappa`),
  exactly like the Monaco/xterm components — the substrate verifies the
  whole VS Code build, not a bare `<script src>`.

**Decision:** The Workspace Projection embeds the **real VS Code web
workbench**, pinned and κ-verified (Law L5), with holospaces-supplied
providers bridging it to the running holospace: a `FileSystemProvider`
over the `virtio-9p` workspace filesystem, a terminal over the holospace
console, and the Language Server / Debug Adapter protocols served by
tools running in the devcontainer OS. Extensions are κ-addressed content
the operator’s `devcontainer.json` selects, imported and verified like
any holospace code (ADR-006). Realized in phases, each witnessed against
its published spec (Chapter 10): `CC-17` the VS Code workbench + the
`FileSystemProvider`/terminal bridge, `CC-18` the Language Server
Protocol intelligence, `CC-19` extensions + the Debug Adapter Protocol.

**Consequences:** The operator gets the authentic VS Code experience —
extensions, command palette, settings, source control, language
intelligence, debugging — and every backend speaks a published protocol
a witness validates (LSP, DAP, the FileSystemProvider API, the Dev
Container spec). The workbench is large; it is imported by κ and
