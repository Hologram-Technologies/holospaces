...
| CC-6 | A recompiled userland runs on the execution surface, on every peer’s engine | the [WebAssembly](https://webassembly.org) specification (validity of the userland + its host-ABI surface) and the [hologram](https://github.com/Hologram-Technologies/hologram) `ContainerRuntime` contract (it boots, suspends, resumes, and migrates) on both the native Wasmtime engine and the `wasmi` interpreter engine (the browser + bare-metal `ContainerEngine`) | surface validation + lifecycle on the real runtime, native and interpreter; a devcontainer holospace boots the userland its config selects (ADR-008) | live (`vv/suites/cc6-execution-surface`) |
...
- **Browser** — `scripts/browser-manager-test.sh`: the Hologram Platform
  ...
  against the host-ABI surface, provisioned, **and booted as a container
  in-browser** through hologram’s `wasmi` interpreter engine (suspended
  to a κ snapshot, resumed, terminated — `CC-6`, ADR-008; an ambient
  import is refused). It is deployed to GitHub Pages by
  `.github/workflows/pages.yml` (the cold-start, untrusted gateway of
  Chapter 6).
...
- **Execution surface** — `cargo test --test cc6_execution_surface`: a
  ...
  migrated on the real `ContainerRuntime` over **both** engines — the
  native Wasmtime JIT and hologram’s `wasmi` interpreter (the browser +
  bare-metal engine) — yielding the same holospace κ on each (Q6). A
  devcontainer holospace boots the userland its config selects
  (`cargo test --test e2e`). The resolved RT1 surface (`CC-6`).
