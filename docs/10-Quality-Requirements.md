...
| CC-9  | A real operating system boots and runs on the emulator                               | the [Linux](https://www.kernel.org) kernel + a real distribution as oracle (the same OS image run natively / under QEMU)                                                                                                                                                                                                                                                       | the system-emulator codemodule boots the κ-addressed image to userspace; a real binary’s output matches the native run; the κ snapshot is reproducible across re-runs and peers                                            | live (`vv/suites/cc9-emulator-os`)       |
| CC-10 | A devcontainer’s OS image + repository ingest as κ content and boot                  | the [Dev Container](https://containers.dev) and OCI image specifications, and the [Linux](https://www.kernel.org) behaviour of the booted environment                                                                                                                                                                                                                          | the image and repo are fetched and verified into κ-addressed content through the substrate (`get_with_fetch`); the same source yields the same holospace κ; the holospace boots and runs the config’s `postCreate`/example | live (`vv/suites/cc10-ingestion`)        |
| CC-11 | The workspace projection renders and drives a running holospace                      | the [Dev Container](https://containers.dev) reference experience and the editor/terminal surface (file content, terminal I/O)                                                                                                                                                                                                                                                  | the editor and file tree read the environment’s content by κ; the operator’s edits and terminal commands publish as canonical events on the holospace’s channels and advance its κ snapshot                                | live (`vv/suites/cc11-workspace`)        |
