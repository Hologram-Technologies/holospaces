...
| `CC-2` `.holo` engine | the native hologram `.holo` executor (`hologram-exec`) as oracle | `Hologram-Technologies/hologram` (pinned rev) | `cc2-holo-engine.sh` → `tests/cc2_holo_engine.rs`: identical `.holo` yields identical κ across independent builds and across the byte- and address-boundary surfaces (determinism + content-addressing). The live browser-vs-native cross-host run belongs to the browser peer's Playwright harness (the hologram `store-opfs` pattern; not yet built — see arc42 ch.11 RT2), sharing this `address_bytes` σ-axis |
