...
| `CC-6` execution surface | the **WebAssembly specification** (validity of the recompiled userland + its closed host-ABI surface) and the **hologram `ContainerRuntime` contract** (the real runtime as the execution oracle) | the WebAssembly spec (as `CC-5`); `Hologram-Technologies/hologram` `hologram-runtime` + `hologram-runtime-wasmtime` (pinned rev) | `cc6-execution-surface.sh` → `tests/cc6_execution_surface.rs`: a κ-addressed Wasm-recompiled userland (the second compute form, ADR-008) validates against the spec + host-ABI surface (`surface::validate_userland` — ambient/WASI imports and missing container-ABI exports are refused), then boots, suspends to a κ snapshot, resumes, and migrates on a real `Runtime` + `WasmtimeEngine`; the same userland κ yields the same holospace (Q6/QS1). No located OCI image (Laws L1/L2/L4) |
