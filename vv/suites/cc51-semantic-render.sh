...
# Upstreams to: holospaces  vv/suites/cc-render.sh holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system(catalog row CC-render, arc42 ch.10).
set -euo pipefail
...
SYSTEM="${HOLO_SYSTEM:-$(cd "$HERE/../artifacts/cc51-55/holo-system" 2>/dev/null && pwd)}"
