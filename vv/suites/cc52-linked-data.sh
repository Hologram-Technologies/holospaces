...
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· W3C Linked Data principles — a URI names a thing and DEREFERENCES to useful data
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  (https://www.w3.org/DesignIssues/LinkedData.html); a κ is expressible as a W3C URI (a did:* DID).
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· multiformats CIDv1 — a sha2-256 κ IS a CIDv1 sha2-256, so κ⇄CID⇄ipfs://⇄/ipfs/ is a bijection
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  (https://github.com/multiformats/cid). The κ⇄CID transform is checked against holo-ipfs (CC-1 axis).
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· Law L5 — every dereferenced hop re-derives or is refused; a tampered target cannot propagate.
