...
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· W3C RDF 1.1 N-Quads (https://www.w3.org/TR/n-quads/) — the interop serialization, round-tripped through
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  the vendored RDF engine.
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· W3C SPARQL 1.1 Basic Graph Pattern matching (https://www.w3.org/TR/sparql11-query/#BasicGraphPatterns) —
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  query() evaluates a BGP SELECT (join core); provably equivalent to SELECT WHERE { …bgp… }.
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· RDF/JS Data Model (http://rdf.js.org/data-model-spec/) — the term model, via vendored N3.js.
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· Law L5 — only re-derived objects materialize; each triple's named graph is its source object's κ
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  (quad-level provenance); the vendored engine (n3.min.js) re-derives to its sealed κ.
