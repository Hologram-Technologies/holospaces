...
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· W3C RDF 1.1 Semantics — the RDFS entailment rules (§9.2.1: rdfs2/3/5/7/9/11) applied as a forward-
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  chaining fixpoint (https://www.w3.org/TR/rdf11-mt/). A derived triple is entailed, not asserted.
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· W3C SHACL (Core) — node-shape validation (sh:targetClass, sh:minCount, sh:class)
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  (https://www.w3.org/TR/shacl/). Full SHACL test suite attaches at promotion.
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system· Law L5 — the "proof" rung: every derived triple's proof chain resolves to SOURCE OBJECT κ that
# holospaces vv/: export HOLO_SYSTEM=<pinned HOLOGRAM>/holo-os/system  re-derive; entailment over verified content, with provenance.
#
